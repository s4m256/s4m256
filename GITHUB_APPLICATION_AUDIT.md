...
| --- | --- | --- | --- |
| Geogebra-Copilot | FEATURE #1 | Public app compiles a semantic triangle with altitude foot and orthocenter; dragging a vertex updates both correctly, with the native toolbar retained. Bridge regression tests cover preserving dependency-based intersections and rejecting an invalid name-inferred fallback. | Public example is fixed input, not live AI generation. Main now validates schemas, ordered dependencies and reference types before compilation; the separate `test` backend still has its own integration boundary. |
...
- **VERIFIED:** GeoGebra main build/lint and 17 tests pass, covering the model-response validation boundary, reference types, generated-name collisions, explicit line aliases and bridge regressions. Public example uses the actual compiler and no provider secret. Pages workflow repeats test/build/lint.
